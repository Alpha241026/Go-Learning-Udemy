...
// function to calculate item price and return (price,found)
	...
	if !found {
		if strings.HasSuffix(itemCode, "_SALE") {
			...
			if found {
				...
				fmt.Printf(" - Item %s (Sale! Orignal: $%.2f, Sale Price: $%.2f)\n",
...
func main() {
	...
	fmt.Printf("----------Processing Order Items----------")
